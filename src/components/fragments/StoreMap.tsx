...
export default function StoreMap(){
  ...
  function ResetView({position,zoom,onReset} : resetView){
    const map = useMap()
    ...
    },[map,position,zoom,onReset])

    return null
  }

  return (
    <div className="[width:clamp(320px,80vw,590px)] sm:[width:clamp(530px,90vw,718px)] md:[width:clamp(691px,90vw,974px)] lg:w-[95%] lg:min-w-[850px] lg:max-w-[1100px] xl:w-[95%] xl:min-w-[1100px] xl:max-w-[1500px] h-[400px] xl:h-[450px] 2xl:h-[550px] shadow-lg overflow-hidden transition-all duration-700 relative">
      <button 
      ...
      {trigger && <ResetView position={initialPosition} zoom={zoom} onReset={() => SetTrigger(false)}/>}
      </MapContainer>
    </div>
  )
}